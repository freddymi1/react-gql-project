...
import { UPDATE_CODE } from '../../../../services/mutations/MutationsCode';

const ExerciseContent = (props) => {
  let [lineCodes, setLineCodes] = useState(JSON.parse(props.codeData.content));

  const [ description, setDescription ] = useState(props.codeData.description);
  const [ languages, setLanguages ] = useState(["html", "css", "javascript"]);
  const [ privacy, setPrivacy ] = useState(props.codeData.privacy);

  const history = useHistory()

  const [ updateCode, { loading, error, data } ] = useMutation(UPDATE_CODE, {
    onCompleted: () => {
      toast.success(<SuccessNotification message="Votre code a ete bien enregistrer!"/>)
      setTimeout(() => {
        history.push("/code")
      }, 1500);
    }
  ...
  const saveWebCode = () => {
    ...
    updateCode({
      variables: {
        id: parseInt(props.codeData.id),
        languages: JSON.stringify(languages),
        description: description,
        content: JSON.stringify(lineCodes),
        privacy: privacy,
        studentId: parseInt(props.codeData.student.id)
      }
    })
  }
  ...
  return (
    <>
      <div className="tab w-full overflow-auto">
        ...
        <div className="w-full overflow-hidden z-10" style={{ position: "absolute", top: "6.5%", bottom: "120px" }}>

          <div className={(showDescription ? "h-full opacity-100" : "h-0 overflow-hidden opacity-0") + " w-full"}>
            <DescriptionEditor description={ description } setDescription={ setDescription } setPrivacy={ setPrivacy } saveWebCode={ saveWebCode }  privacy={ privacy } />
          </div>

          <div className={(showCode ? "h-full opacity-100" : "h-0 overflow-hidden opacity-0") + " w-full"}>
            <CodeEditor lineOfCode={lineCodes} handleCodeChange={handleCodeChange} setPrivacy={ setPrivacy }  privacy={ privacy }  saveWebCode={ saveWebCode } />
          </div>
          <div className={(showResult ? "h-full opacity-100" : "h-0 overflow-hidden shadow opacity-0") + " w-full bg-white"}>
            <div className="bg-gray-500 w-full px-6 py-3">
              <Title setPrivacy={ setPrivacy } saveWebCode={ saveWebCode } privacy={ privacy } />
            </div>
            <iframe className="block w-full h-full shadow-lg border-2 border-gray-200" id="runCode"></iframe>
          </div>
        </div>
        <div className="w-full bg-gray-500 flex h-14 pr-5 text-gray-700 float-right" style={{ position: "absolute", bottom: "5.75%" }}>
          ...
          >
            <FiFileText className="inline mr-2" size="2em" /><span>Description</span>
          </button>
          <button className="block px-6 py-3 w-full text-gray-50 bg-transparent hover:shadow-md outline-none focus:outline-none"
            onClick={
              e => {
