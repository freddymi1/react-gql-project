...
class WebCodeEditor extends React.Component {
  ...
  componentDidMount() {
    this.props.lineOfCode.map(loc => {
      if(loc.language === "css"){
        this.setState({ myCodeCss: loc.code })
      } else if(loc.language === "javascript"){
        this.setState({ myCodeJs: loc.code })
      } else {
        this.setState({ myCodeHtml: loc.code })
      }
    })
  }
