...
import { COMMENT_VOTE_BY_COMMENT_ID, COMMENT_VOTE_BY_STUDENT_ID_AND_COMMENT_ID } from "../../../../../../../../services/queries/CommentVoteQueries";
import { CREATE_COMMENT_VOTE, DESTROY_COMMENT_VOTE } from "../../../../../../../../services/mutations/CommentVoteMutations"
...
const CommentList = (props) => {
  ...
  const { data: cvbsacData } = useQuery(
    ...
    {
      variables: { commentId: props.comment.comment.id, studentId: props.studentId }
    }
  )
  ...
  useEffect(() => {
    if(commentVoteByCommentIdData && voteIsUpdated === false){
      ...
      if(cvbsacData && cvbsacData.commentVoteByStudentIdAndCommentId !== null) {
                    ...
                    <div className="reponse_title">
                        <div className='text-gray-600'>
                            <button onClick={
                                e => {
                                if(unvote === false){
                                    createCommentVote({ ignoreResults: false, variables: { commentId: props.comment.comment.id, studentId: props.studentId } })
                            ...
                            <button onClick={
                                e => {
                                if(unvote === true){
                                    destroyCommentVote({ variables: { commentId: props.comment.comment.id, studentId: props.studentId } })
                                    setVoteCount(voteCount - 1)
                                    setUnvote(false)
                                }
                                }
                        ...
                        <label className={`responsebtn ${active} cursor-pointer text-sm text-gray-500 ml-4 rounded-sm px-2 py-1 shadow-sm`} onClick={toggleAccordion}>
                            Reponse
                        </label>
                    </div>
                    ...
                    <div className=" px-4">
                      <div className="w-full overflow-y-auto">
                        <div className="p-4 mb-4 bg-gray-200">
                          <label className="text-gray-500 text-md font-semibold p-4">Liste des reponses associe a cette commentaire</label>
                        </div>
                        <ReponseList/>
                        <ReponseList/>
                      </div>
